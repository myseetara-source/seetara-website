...
import { Flame, ShoppingCart, Smartphone } from 'lucide-react';
...
export default function Header({ onClaimOffer }: HeaderProps) {
  ...
  useEffect(() => {
    const DURATION = (3 * 60 * 60 * 1000) + (45 * 60 * 1000) + (30 * 1000); 
    const STORAGE_KEY = 'seetara_flash_sale_sw101_v1';
  ...
  return (
    <>
      {/* Urgency Banner */}
      <div className={`bg-gradient-to-r from-amber-600 via-orange-500 to-red-500 text-white text-center py-2 px-3 font-bold z-[60] fixed top-1 left-0 right-0 flex items-center justify-center transition-all ${isFlashing ? 'scale-105' : ''}`}>
