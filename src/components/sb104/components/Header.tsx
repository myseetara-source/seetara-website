...
import { Flame, ShoppingCart, Briefcase } from 'lucide-react';
...
export default function Header({ onClaimOffer }: HeaderProps) {
  const [timeLeft, setTimeLeft] = useState({ hours: 5, minutes: 59, seconds: 45 });
  ...
  useEffect(() => {
    const DURATION = (5 * 60 * 60 * 1000) + (59 * 60 * 1000) + (45 * 1000); 
    const STORAGE_KEY = 'seetara_flash_sale_sb104_v1';
  ...
  return (
    <>
      {/* Urgency Banner */}
      <div className={`bg-gradient-to-r from-emerald-700 via-teal-600 to-emerald-700 text-white text-center py-2 px-3 font-bold z-[60] fixed top-1 left-0 right-0 flex items-center justify-center transition-all ${isFlashing ? 'scale-105' : ''}`}>
        <div className="flex items-center justify-center gap-2">
          <Flame className="w-4 h-4 animate-pulse flex-shrink-0" />
          <div className="flex items-center gap-1.5 text-[11px] sm:text-xs">
            <span className="font-bold">💼 WORK BAG SALE!</span>
      ...
      <header className="bg-white shadow-md py-2.5 px-4 sticky top-9 z-40 border-b border-gray-100">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          ...
          <button 
            ...
            className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white px-4 py-2 rounded-full font-bold text-xs shadow-lg active:scale-95 transition-all flex items-center gap-1.5"
          >
