...
export default function LunaLandingPage() {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach(entry => {
          // Hide bottom bar when form is even slightly visible (10%)
          setIsFormVisible(entry.isIntersecting);
        });
      },
      { 
        threshold: 0.1,  // Lower threshold - hide bottom bar sooner
        rootMargin: '-50px 0px 0px 0px'  // Start hiding when form is 50px from viewport top
      }
    );
  ...
  // Fluctuate viewers (min 50, max 600)
  useEffect(() => {
    const interval = setInterval(() => {
      setViewers(prev => {
        const change = Math.floor(Math.random() * 20) - 8; // -8 to +11
        const newValue = prev + change;
        return Math.max(50, Math.min(600, newValue)); // Keep between 50-600
      });
    }, 5000);
    return () => clearInterval(interval);
  }, []);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const orderId = `luna_${formData.phone}_${Date.now()}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-orange-50 overflow-x-hidden">
      {/* Scroll Progress Bar - Luna themed */}
      <div className="fixed top-0 left-0 right-0 h-1 bg-amber-100 z-[70]">
        <div 
          className="h-full bg-gradient-to-r from-amber-500 via-orange-500 to-amber-500 transition-all duration-150 ease-out"
        ...
        >
          ...
          {showScrollHint && (
            <div className="flex flex-col items-center justify-center py-4 animate-fade-in">
              <p className="text-xs text-amber-600 mb-2 tracking-widest uppercase">Scroll to explore</p>
              <div className="w-7 h-12 border-2 border-amber-300 rounded-full flex items-start justify-center p-1.5 relative">
                <div className="w-2 h-3 bg-gradient-to-b from-amber-500 to-orange-500 rounded-full animate-scroll-mouse"></div>
              </div>
              <div className="flex flex-col items-center mt-1">
                <ChevronDown className="w-4 h-4 text-amber-400 animate-bounce" style={{ animationDelay: '0s' }} />
                <ChevronDown className="w-4 h-4 text-amber-300 -mt-2 animate-bounce" style={{ animationDelay: '0.1s' }} />
              </div>
            </div>
          )}
        </section>
