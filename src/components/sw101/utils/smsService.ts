 ...
 */
const sendSMS = async (to: string, text: string, authToken: string): Promise<unknown> => {
  try {
...
const createSalesTeamMessage = (orderData: OrderData): string => {
  ...
  const productSKU = `SW101-${color}`;
  
  if (orderType === 'inquiry') {
    return `NEW INQUIRY - SW101!
...
export const sendOrderNotifications = async (
  ...
  try {
    ...
    if (results.customer && (results.customer as { error?: boolean }).error === false) {
