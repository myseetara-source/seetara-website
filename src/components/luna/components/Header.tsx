...
import { Flame, ShoppingCart, Moon } from 'lucide-react';
...
export default function Header({ onClaimOffer }: HeaderProps) {
  ...
  useEffect(() => {
    const DURATION = (5 * 60 * 60 * 1000) + (45 * 60 * 1000) + (30 * 1000); 
    const STORAGE_KEY = 'seetara_flash_sale_luna_v1';
  ...
  return (
    <>
      {/* Urgency Banner - Luna themed warm colors */}
      <div className={`bg-gradient-to-r from-amber-600 via-orange-500 to-amber-600 text-white text-center py-2 px-3 font-bold z-[60] fixed top-1 left-0 right-0 flex items-center justify-center transition-all ${isFlashing ? 'scale-105' : ''}`}>
        <div className="flex items-center justify-center gap-2">
          <Moon className="w-4 h-4 animate-pulse flex-shrink-0 text-yellow-200" />
          <div className="flex items-center gap-1.5 text-[11px] sm:text-xs">
            <span className="font-bold">🌙 LUNA OFFER!</span>
            <div className="flex items-center gap-0.5 bg-black/20 px-2 py-0.5 rounded font-mono">
          ...
          <Flame className="w-4 h-4 animate-pulse flex-shrink-0 text-yellow-200" />
        </div>
      </div>
      ...
      {/* Main Header - Light theme for Luna */}
      <header className="bg-white shadow-md py-2.5 px-4 sticky top-9 z-40 border-b border-amber-100">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          ...
          {/* CTA Button - Luna themed */}
          <button 
            onClick={onClaimOffer} 
            className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-4 py-2 rounded-full font-bold text-xs shadow-lg active:scale-95 transition-all flex items-center gap-1.5"
          >
