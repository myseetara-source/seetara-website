...
const Hero = ({ 
  ...
  const handleNext = () => {
    ...
    setSelectedColorIndex((prev) => (prev + 1) % productColors.length);
    setShowVideo(false); 
  ...
  const handlePrev = () => {
    ...
    setSelectedColorIndex((prev) => (prev - 1 + productColors.length) % productColors.length);
    setShowVideo(false);
    setTimeout(() => setIsAnimating(false), 300);
  };
  ...
  return (
    <div className="px-4 pt-0 pb-2 overflow-hidden mt-2 md:px-0 md:mt-0"> 
      {/* Main Image Card - 3:4 ratio for video, square for images */}
              ...
              <div className="w-full h-full animate-float group-hover:scale-110 transition-transform duration-700 ease-in-out relative">
                ...
                {imageError ? (
                  ...
                  <img 
                    ...
                    className={`w-full h-full object-contain p-2 ${slideDirection === 'right' ? 'slide-right' : 'slide-left'} ${imageLoading ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}
                    loading="eager"
